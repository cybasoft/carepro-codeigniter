...
INSERT INTO options (id, option_name, option_value, autoload) VALUES
  (1, 'company_name', 'DaycarePRO', NULL),
  (2, 'slogan', 'daycare management system', NULL),
  (3, 'email', 'app@admin.com', NULL),
  (4, 'phone', '12345677', NULL),
  (5, 'fax', '1234566', NULL),
  (6, 'street', '1245 State st', NULL),
  (7, 'street2', '', NULL),
  (8, 'city', 'New York', NULL),
  (9, 'state', 'NY', NULL),
  (10, 'postal_code', '12345', NULL),
  (11, 'country', 'USA', NULL),
  (12, 'timezone', 'America/New_york', NULL),
  (13, 'google_analytics', '', NULL),
  (14, 'currency_symbol', '$', NULL),
  (15, 'date_format', 'd/m/Y h:i a', NULL),
  (16, 'allow_registration', '0', NULL),
  (17, 'allow_reset_password', '1', NULL),
  (18, 'enable_captcha', '0', NULL),
  (19, 'demo_mode', '0', NULL),
  (20, 'maintenance_mode', '0', NULL),
  (21, 'use_smtp', '1', NULL),
  (22, 'smtp_user', '', NULL),
  (23, 'smtp_pass', '', NULL),
  (24, 'smtp_host', '', NULL),
  (25, 'smtp_port', '25', NULL),
  (26, 'logo', 'logo.png', NULL),
  (27, 'invoice_logo', 'invoice_logo.png', NULL),
  (28, 'stripe_pk_live', '', NULL),
  (29, 'stripe_sk_live', '', NULL),
  (30, 'stripe_pk_test', '', NULL),
  (31, 'stripe_sk_test', '', NULL),
  (32, 'paypal_email', '', NULL),
  (33, 'paypal_locale', '', NULL),
  (34, 'page', 'settings', NULL),
  (35, 'currency_abbreviation', 'USD', NULL),
  (36, 'logo_bg_color', '', NULL),
  (37, 'top_nav_bg_color', '', NULL),
  (38, 'top_nav_link_color', '', NULL),
...
(
  ...
  staff_only TINYINT DEFAULT '0' NULL,
  remarks    VARCHAR(255)        NOT NULL,
  given_at   DATETIME            NOT NULL,
  created_at DATETIME            NOT NULL,
  CONSTRAINT meds_admin_ibfk_1
  FOREIGN KEY (med_id) REFERENCES child_meds (id)
    ON UPDATE CASCADE
    ON DELETE CASCADE,
  CONSTRAINT meds_admin_ibfk_3
  FOREIGN KEY (med_id) REFERENCES child_meds (id)
    ON UPDATE CASCADE
    ON DELETE CASCADE,
  CONSTRAINT meds_admin_ibfk_6
  FOREIGN KEY (med_id) REFERENCES child_meds (id)
    ON UPDATE CASCADE
    ON DELETE CASCADE,
  CONSTRAINT meds_admin_ibfk_2
  FOREIGN KEY (user_id) REFERENCES users (id)
    ON UPDATE CASCADE
    ON DELETE CASCADE,
  CONSTRAINT meds_admin_ibfk_4
  FOREIGN KEY (user_id) REFERENCES users (id)
    ON UPDATE CASCADE
    ON DELETE CASCADE,
  CONSTRAINT meds_admin_ibfk_7
  FOREIGN KEY (user_id) REFERENCES users (id)
    ON UPDATE CASCADE
    ON DELETE CASCADE,
  CONSTRAINT meds_admin_ibfk_5
  FOREIGN KEY (child_id) REFERENCES children (id)
    ON UPDATE CASCADE
    ON DELETE CASCADE,
  CONSTRAINT meds_admin_ibfk_8
  FOREIGN KEY (child_id) REFERENCES children (id)
    ON UPDATE CASCADE
    ON DELETE CASCADE
);
...
CREATE INDEX child_id
  ON meds_admin (child_id);

CREATE INDEX med_id
  ON meds_admin (med_id);

CREATE INDEX user_id
  ON meds_admin (user_id);
...
CREATE UNIQUE INDEX notes_categories_name_uindex
  ON notes_categories (name);

ALTER TABLE child_notes
  ADD COLUMN category_id INT UNSIGNED NOT NULL;

ALTER TABLE `child_notes`
  ADD FOREIGN KEY (`category_id`) REFERENCES notes_categories (`id`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;

INSERT INTO notes_categories (name) VALUES
  ('Academics'),
  ('Arts & Crafts'),
  ('Circle Time'),
  ('Physical Fitness'),
...
CREATE UNIQUE INDEX notes_tags_name_uindex
  ON notes_tags (name);
