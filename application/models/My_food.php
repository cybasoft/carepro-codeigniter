...
class My_food extends CI_Model
{
    ...
    function newPref()
    {
        ...
        if($this->db->insert('child_foodpref', $data)) {
            ...
            //log
            logEvent($id = NULL,"Added food pref ID: {$last_id} for child ID: {$this->input->post('child_id')}",$care_id = NULL);
            //notify parent
    ...
    function recordIntake()
    {
        ...
        if($this->db->affected_rows() > 0) {
            $last_id = $this->db->insert_id();
            logEvent($user_id = user_id(),"Added food intake record ID: {$last_id} for child ID: {$child_id}",$care_id = NULL);
    ...
    function updateFoodReport($data)
    {
        ...
        if(count((array)$res) > 0) { //record exists then update
            ...
            $this->db
                ...
                logEvent($user_id = NULL,"Food intake updated for form_ny_attendance of child ID: {$data['child_id']}",$care_id = NULL);
        } else {
            ...
            $last_id = $this->db->insert_id();
            logEvent($user_id = NULL,"Food intake ID: {$last_id} added for form_ny_attendance of child ID: {$data['child_id']}",$care_id = NULL);
        }
    }
