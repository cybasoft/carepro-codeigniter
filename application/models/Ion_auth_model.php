...
class Ion_auth_model extends CI_Model
{
	...
	public function users($groups = NULL)
	{
		...
		if(isset($this->_ion_select) && !empty($this->_ion_select)) {
			...
			//default selects
			if($this->conf->logged_in() ==true){
				$this->db->where('company', $this->company->company()->id);
			}
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		//log event
		$this->conf->log("updated {$username}");
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, company')
			->where($this->identity_column, $identity)
			->limit(1)
			->get($this->tables['users']);
	...
	public function update($id, array $data)
	{
		...
		if(array_key_exists($this->identity_column, $data) && $this->identity_check($data[$this->identity_column]) && $user->{$this->identity_column} !== $data[$this->identity_column]) {
            ...
            //$this->conf->msg('danger','Email is already in use');
	...
	public function delete_user($id)
	{
		...
		//log event
		$this->conf->log("Deleted {$this->users->user($id)->username}");
