...
class UserController extends CI_Controller
{
    ...
    function index()
    {
        ...
        $users = $this->db->select('u.*,u.id as user_id,ad.id as add_id,ug.group_id,g.name as role,ad.*')
            ...
            ->join('address as ad', 'ad.id=u.address_id', 'left')
            ->get()->result();
    ...
    function update($id = NULL)
    {
        ...
        if (
            $groupData[0] == 1 || $groupData[0] == 2 || $groupData[0] == 4 || ($groupData[0] == 3 && ($staff_count < $plan_staff_count || $plan_staff_count == $UNLIMITED))
        ) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $user = $this->db->where('id', $id)->update('users', $data);
                ...
                if ($user) {
                    //update address
                    $address_id = $this->db->where('id', $id)->get('users')->row()->address_id;

                    if (empty($address_id)) {
                        $this->db->insert('address', $addressData);
                        $address_id = $this->db->insert_id();

                        $this->db->where('id', $id)->update('users', ['address_id' => $address_id]);
                    } else {
                        $this->db->where('id', $address_id)->update('address', $addressData);
                    }
