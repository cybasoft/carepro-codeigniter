...

{
...

	{
		...
		if ($this->form_validation->run() == true) {
			...
			$this->email->from($email, 'Web User');

			$this->email->to('jgmuchiri@gmail.com');

			$this->email->cc('info@icoolpix.com');
			...
			if ($this->email->send()) {
				...
				$this->conf->msg('success', 'Thank you! We will keep you in the loop of what is happening here!');

			} else {

				$this->conf->msg('danger', 'An error has occurred! Please try again');
...

	{
		...
		if ($this->form_validation->run() == true) {
			...
			if ($this->email->send()) {
				...
				$this->conf->msg('success', 'Thank you! Your message has been sent!');

			} else {

				$this->conf->msg('danger', 'An error has occurred! Please try again or contact us by another means');
...

	{
		...
		if ($this->conf->logged_in()) :

			$this->conf->page('errors/404_in');
