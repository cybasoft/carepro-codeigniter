...
//        $room = $this->db->where('id', $id)->get('child_rooms')->row();
    ...
    function destroy()
    {
        ...
        logEvent($user_id = NULL, "Deleted room ID: {$id}",$care_id = NULL);
    ...
    function assignChildren()
    {
        ...
        if($this->form_validation->run() == TRUE) {
            ...
            foreach ($this->input->post('child_id') as $child) {
                ...
                logEvent($user_id = NULL,"Assigned child ID: {$child} for room ID: {$room}",$care_id = NULL);
            }
    ...
    function assignStaff()
    {
        ...
        if($this->form_validation->run() == TRUE) {
            ...
            foreach ($this->input->post('user_id') as $user) {
                ...
                logEvent($user_id = NULL, "Assigned staff user ID: {$user} for room ID: {$room}",$care_id = NULL);
            }
    ...
    function deleteNote()
    {
        ...
        logEvent($user_id = NULL, "Deleted note ID: {$id} for room ID: {$room_notes->room_id}",$care_id = NULL);
        flash('success', lang('request_success'));
    ...
    function detachStaff()
    {
        ...
        logEvent($id = NULL,"Detached staff user ID: {$user_id} for room ID: {$room_id}",$care_id = NULL);
        flash('success', lang('request_success'));
    ...
    function detachChild()
    {
        ...
        logEvent($user_id = NULL, "Detached child ID: {$child_id} for room ID: {$room_id}",$care_id = NULL);
        flash('success', lang('request_success'));
