...
class Health extends CI_Controller
{
    ...
    function deleteAllergy($id)
    {
        ...
        if($this->db->affected_rows() > 0) {
            logEvent($user_id=NULL,"Deleted allergy ID: {$id} for child ID: {$allergies->child_id}",$care_id = NULL);
            flash('success', lang('request_success'));
        } else {
            flash('danger', lang('request_error'));
        }
    ...
    function deleteContact($id)
    {
        ...
        if($this->db->where('id', $id)->delete('child_contacts')) {
            logEvent($user_id = NULL,"Deleted child contact ID: {$id} for child ID: {$contacts->child_id}",$care_id = NULL);
            flash('success', lang('request_success'));
        } else {
    ...
    function deleteProvider($id)
    {
        ...
        if($this->db->where('id', $id)->delete('child_providers')) {
            logEvent($user_id = NULL,"Deleted provider ID: {$id} for child ID: {$providers->child_id}",$care_id = NULL);
            flash('success', lang('request_success'));
        } else {
            flash('danger', lang('request_danger'));
        }
    ...
    function deleteProblem($id)
    {
        ...
        if($this->db->where('id', $id)->delete('child_problems')) {
            logEvent($user_id = NULL,"Deleted child problem ID: {$id} for child ID: {$problems->child_id}",$care_id = NULL);
            flash('success', lang('request_success'));
        } else {
            flash('danger', lang('request_danger'));
        }
